...
export const createDynamicFormSchema = (formStepData: FormStepData[]) => {
	...
	validatableSteps.forEach((step) => {
		...
		switch (step.type) {
			...
			case "ADDRESS":
				...
				const contactInfoSchema = z.record(z.string(), z.object({
					value: z.string(),
					title: z.string()
				}));
				...
				if (step.data?.fields && Array.isArray(step.data.fields)) {
					const refinedSchema = contactInfoSchema.refine((data) => {
						const errors: string[] = [];
						...
						step.data.fields.forEach((field: any) => {
							...
							if (fieldValue && field.type === "email") {
								...
								if (!emailRegex.test(fieldValue)) {
									errors.push(`Please enter a valid email address for ${field.title || field.id}`);
								}
							}
							...
							if (fieldValue && field.type === "tel") {
								...
								if (!phoneRegex.test(fieldValue.replace(/[\s\-\(\)]/g, ""))) {
									errors.push(`Please enter a valid phone number for ${field.title || field.id}`);
								}
							}
						});
					...
					schemaFields[fieldKey] = step.required ? refinedSchema : refinedSchema.optional();
				} else {
					schemaFields[fieldKey] = step.required ? contactInfoSchema : contactInfoSchema.optional();
				}
				break;
			...
			case "MULTI_SELECT_OPTION":
				...
				if (step.data?.selectionType === "fixed" && step.data?.fixedSelections) {
				...
				schemaFields[fieldKey] = step.required ? multiSelectSchema : multiSelectSchema.optional().default([]);
				break;
			...
			case "NUMBER":
				...
				schemaFields[fieldKey] = step.required ? numberSchema : numberSchema.optional();
				break;
			...
			case "LONG_TEXT":
				...
				schemaFields[fieldKey] = step.required ? longTextSchema : longTextSchema.optional();
				break;
			...
			case "SHORT_TEXT":
				...
				schemaFields[fieldKey] = step.required ? shortTextSchema : shortTextSchema.optional();
				break;
			...
			default:
				...
				schemaFields[fieldKey] = step.required ? defaultSchema : defaultSchema.optional();
				break;
		}
	});
...
export type DynamicFormData = z.infer<ReturnType<typeof createDynamicFormSchema>>;